/* ============================================================
   ...
   Version: 7.0 – Final Production (No Loader, Two-Hero Ready)
   ============================================================ */
...
/**
 * Add event listener with automatic cleanup reference (used internally)
 */
...
/* ============================================================
   2. CURSOR CLASS – with visibility API pause + cleanup
   ============================================================ */
class Cursor {
    constructor() {
    ...
    bindEvents() {
        ...
        // Event delegation for hover effects (enlarge on interactive elements)
        const onMouseEnter = (e) => {
            const target = e.target.closest('button, a, .collection__card, .brands__item, .hero__btn, .about__btn, .hero-second__btn');
...
/* ============================================================
   3. HEADER CLASS – using IntersectionObserver for active link
   ============================================================ */
class Header {
    constructor() {
...
/* ============================================================
   4. SCROLL REVEAL – using Set to avoid duplicates + cleanup
   ============================================================ */
class ScrollReveal {
    constructor() {
...
/* ============================================================
   5. CARD TILT – shared single RAF loop for all cards
   ============================================================ */
class CardTilt {
    constructor() {
    ...
    init() {
        for (const card of this.cards) {
            ...
            const onMouseLeave = () => {
                ...
                data.currentX = 0;
                data.currentY = 0;
                // Check if any card is still hovering
                if (!this.cardData.some(d => d.isHovering)) {
                    this.active = false;
                    if (this.rafId) {
                        cancelAnimationFrame(this.rafId);
                        this.rafId = null;
    ...
    loop() {
        ...
        for (const data of this.cardData) {
            if (!data.isHovering) continue;
            anyActive = true;
            const card = data.element;
            const rect = data.rect || card.getBoundingClientRect();
            ...
            const rotateX = ((data.currentY - centerY) / centerY) * -4;
            const rotateY = ((data.currentX - centerX) / centerX) * 4;
...
/* ============================================================
   6. RIPPLE EFFECT – with object pooling for performance
   ============================================================ */
class Ripple {
    constructor() {
        this.buttons = safeSelectorAll('.hero__btn, .about__btn, .collection__card-link, .hero-second__btn');
    ...
    init() {
        for (const btn of this.buttons) {
            ...
            const onClick = (e) => {
                ...
                // Get ripple from pool or create new
                let ripple = this.pool.pop();
                if (!ripple) {
...
/* ============================================================
   7. HERO PARALLAX – for hero background image
   ============================================================ */
class HeroParallax {
    constructor() {
        // Target the hero background image (if exists)
        this.heroBg = safeSelector('.hero__bg-image') || safeSelector('.hero__picture');
    ...
    init() {
        const handler = throttle(() => {
            ...
            const offset = Math.min(scrollY * 0.06, maxScroll);
            this.heroBg.style.transform = `translateY(${offset}px)`;
        }, 30);
        window.addEventListener('scroll', handler, { passive: true });
        this.cleanupFns.push(() => window.removeEventListener('scroll', handler));
        handler();
...
/* ============================================================
   8. HERO MOUSE GLOW – with rAF and cleanup
   ============================================================ */
class HeroMouseGlow {
    constructor() {
...
/* ============================================================
   10. SMOOTH SCROLL – (only for non-header links, to avoid duplication)
   ============================================================ */
class SmoothScroll {
    constructor() {
        // Exclude header links (they are handled by Header class)
        this.links = safeSelectorAll('a[href^="#"]:not(.header__nav-link)');
    ...
    init() {
        for (const link of this.links) {
            // Skip footer logo (already handled by header or manual)
            if (link === safeSelector('.footer__logo')) continue;
...
/* ============================================================
   13. ACCESSIBILITY – CSS-based, minimal JS
   ============================================================ */
class Accessibility {
    constructor() {
        this.cleanupFns = [];
...
/* ============================================================
   14. PERFORMANCE MONITOR – robust using navigation entry
   ============================================================ */
...
/* ============================================================
   15. MAIN APP – orchestrates everything with full cleanup
   ============================================================ */
class App {
    constructor() {
        this.modules = [];
    ...
    init() {
        // Order: accessibility first, then monitor
